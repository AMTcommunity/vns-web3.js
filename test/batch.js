    ...
    describe('execute', function () {
        it('should execute batch request', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'vns_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'vns_getBalance');
                assert.deepEqual(second.params, ['0x0000000000000000000000000000000000000005', 'latest']);
            });

            var batch = new web3.BatchRequest();
            batch.add(web3.vns.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(web3.vns.getBalance.request('0x0000000000000000000000000000000000000005', 'latest', callback2));
            batch.execute();
        });
        ...
        it('should execute batch request for async properties', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'vns_accounts');
            ...
            var batch = new web3.BatchRequest();
            batch.add(web3.vns.getAccounts.request(callback));
        ...
        it('should execute batch request with contract', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(payload[2].method, 'vns_call');
                ...
                assert.equal(payload[3].method, 'vns_call');
                ...
                assert.equal(payload[4].method, 'vns_call');
            ...
            var batch = new web3.BatchRequest();
            batch.add(web3.vns.getBalance.request('0x0000000000000000000000000000000000000022', 'latest', callback));
            batch.add(new web3.vns.Contract(abi, address).methods.balance(address).call.request(callback2));
            batch.add(new web3.vns.Contract(abi, address).methods.balance(address).call.request({from: '0x1000000000000000000000000000000000000002'}, callback2));
            batch.add(new web3.vns.Contract(abi, address).methods.balance(address).call.request({from: '0x1000000000000000000000000000000000000003'}, 10, callback2));
            batch.add(new web3.vns.Contract(abi, address).methods.balance(address).call.request(10, callback3));
        ...
        it('should execute batch requests and receive errors', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'vns_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'vns_call');
                assert.deepEqual(second.params, [{
                    'to': '0x1000000000000000000000000000000000000001',
            ...
            var batch = new web3.BatchRequest();
            batch.add(web3.vns.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(new web3.vns.Contract(abi, address).methods.balance(address).call.request({from: '0x0000000000000000000000000000000000000000'}, 10, callback2));
        ...
        it('should propagate output formatter error to callback', function(done) {
            ...
            provider.injectValidation((payload) => {
                assert.equal(payload[0].method, 'vns_call');
            ...
            batch.add(new web3.vns.Contract(abi, address)
                .methods.symbol()
